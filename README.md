...
阶段 1 到阶段 5 的代码与自动验证现已完成：Web 可以安全暂存单个 TXT、Markdown
...
继续执行 Prompt、ChatModel、字符串解析、格式归一化和来源返回。
...
Crawl4AI 转换与 Fake/Mock 安全验证；真实公开网页预览、付费确认入库和问答闭环仍需
单独验收，不能用自动测试替代。Agent 和 LangGraph 仍属于后续规划。
...
`web_preview`、`web_preview_protected`、`client_error`、`server_error` 和
...
本地单进程使用一个非阻塞独占保护器协调问答、网页预览、暂存、索引和删除：已有受保护操作
...
当前 API 已覆盖本地问答、上传与增量索引、资料删除、有限请求历史和单进程保护，但
...
截图为准。显式 LCEL 固定问答管道和受限 Crawl4AI 网页预览已经实现；当前项目仍没有
LangChain Agent 或 LangGraph 实现，`requirements.txt` 中没有 LangGraph 依赖。
LCEL 只重组现有固定问答流程，不能描述为已经具备动态工具选择或有状态工作流。
...
| **当前已完成 13：阶段 5 Crawl4AI 网页资料导入** | 公网 URL/DNS/每跳重定向校验；固定到已验证 IP 的受控单页抓取；Crawl4AI 本地 Markdown 预览；来源元数据；复用现有暂存与确认索引链路。 | Fake/Mock 覆盖 SSRF、重定向、超限、错误映射和隐私日志；本地原始 HTML 已实际通过 Crawl4AI 清理转换，API/UI 契约已自动验证；真实公网 IP 的 HTTPS 首跳和重定向再校验已观察到。 | 当前环境把域名映射到保留测试网段，因此尚未完成真实域名的 Markdown 页面/API 验收，也未确认付费入库和问答；不支持 JavaScript 渲染、登录态或批量爬取。 |

### 下一阶段与后续阶段

| 阶段 | 主要内容 | 完成标志 | 与前一阶段的依赖 / 核心风险与边界 |
| --- | --- | --- | --- |
| **阶段 6（下一阶段）：LangChain Agent 工具编排** | 使用当前官方推荐的 `create_agent`，不使用旧式 Agent 构造方式；至少有两个真实、边界清晰的工具后再引入。候选工具为 `answer_from_materials`（调用 LCEL RAG）、`list_available_materials`（列出资料）和 `preview_web_material`（只生成网页预览，不直接入库）。 | Agent 能根据用户意图选择正确工具；普通资料问答继续复用稳定的 LCEL RAG；工具失败时安全停止；工具选择和结果有 Fake/Mock 测试。 | 依赖阶段 4 的 LCEL RAG 和阶段 5 的安全预览能力；在进入实现前，先完成阶段 5 的真实公开页面预览验收。写入 Chroma、批量抓取或明显产生费用的操作必须显式确认。 |
| **阶段 7（后续阶段）：LangGraph 学习规划工作流** | 使用 `StateGraph` 明确定义 State、Nodes 和 Edges；支持学习目标、任务拆解、资料检索与引用、用户确认、进度状态、阶段复盘、条件分支、有限重试和状态恢复；将 Agent 作为受控节点或子图，而不是交出全部流程控制权。 | 展示“学习目标 → 任务拆解 → 资料检索 → 用户确认 → 进度更新”的完整状态流；中断后可从已有状态恢复；每次状态变化和路由原因可解释；人工拒绝后不会继续写入或调用付费能力。 | 依赖阶段 6 的受控工具边界；生成学习计划、批量导入网页或修改进度前设置人工确认节点，并使用可持久化检查点支持中断恢复。第一版只做单 Agent 或少量受控节点，不为展示技术栈构建复杂多 Agent 系统。 |
