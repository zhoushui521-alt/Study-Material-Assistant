...
| 步骤 | 时间 | 展示内容 | 现场边界 |
| --- | --- | --- | --- |
| 1. 项目介绍 | 00:00～01:10 | 产品问题、可信回答定位、登录与用户空间 | 不把登录自动化说成多用户压测 |
| 2. 上传学习资料 | 01:10～01:30 | 文件选择、本地安全校验、解析结果 | 零 Embedding / Chat 调用 |
| 3. 知识库构建 | 01:30～01:50 | Chunk / 批次估算、202 + job_id 与任务状态 | 默认不确认费用；使用预索引资料继续 |
| 4. RAG 回答 | 01:50～02:30 | 提交资料内问题并展示回答 | 只执行彩排过的一次问答 |
| 5. Citation | 02:30～03:05 | 核对资料名、摘录、locator 与 Request ID | 有效 Citation ID 不等于 claim-level 支持已评测 |
| 6. Tutor Agent | 03:05～04:15 | 解释或练习路径，说明复用同一 RAGService | 按实际路由说明是否产生 Citation |
| 7. Model Gateway | 04:15～05:00 | 只读凭证元数据、Provider 来源与匿名 Metrics | 不展示 Key，不虚构前端切换控件 |

步骤 2 与步骤 3 必须分开理解：上传暂存是本地预检，知识库构建才可能触发 Embedding。
安全演示只展示估算与既有任务结果；只有在完整任务模式、已确认调用数量和费用时才现场创建 Job。
